...
@if(in_array(Auth::user()->role_id, [3, 4]))
    <div x-show="tab === 'owner-deals'" class="bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-800 rounded-2xl p-6 md:p-8 shadow-sm space-y-6">
        ...
        <div class="space-y-8">
            ...
            <div>
                <h3 class="font-bold text-sm text-slate-800 dark:text-slate-200 border-b border-slate-100 dark:border-zinc-800 pb-2 mb-4">Pengajuan Sewa Properti</h3>
            ...
            <div>
                <h3 class="font-bold text-sm text-slate-800 dark:text-slate-200 border-b border-slate-100 dark:border-zinc-800 pb-2 mb-4">Pengajuan Beli Properti</h3>
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-sm text-slate-500 dark:text-slate-400">
                        <thead class="text-xs uppercase text-slate-400 bg-slate-50 dark:bg-zinc-950">
                            <tr>
                                    ...
                                    <td class="px-4 py-3 text-xs flex items-center gap-3">
                                        ...
                                        <div class="flex flex-col gap-1.5">
                                            ...
                                            @elseif($tx->status === 'booking')
                                                <button onclick="updateDealStatus('transaction', {{ $tx->id }}, 'menunggu_pelunasan')" class="py-1.5 px-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg font-bold shadow transition-all active:scale-95 text-[10px]" title="Tagih Pelunasan">
                                                    Tagih Pelunasan (PPJB Siap)
                                                </button>
                                            @elseif($tx->status === 'menunggu_pelunasan')
                                                <span class="text-slate-500 dark:text-slate-400 font-medium text-[10px]">Menunggu Pelunasan Pembeli</span>
                                            @endif
                                        </div>
